...
function battery_widget:init(args)
  ...
  self.percent_colors = args.percent_colors
    or args.limits
    or {
      { 25, "red" },
      { 50, "orange" },
  ...
  self.widget_text = args.widget_text or "${AC_BAT}${color_on}${percent}%${color_off}"
  self.tooltip_text = args.tooltip_text or "Battery ${state}${time_est}\nCapacity: ${capacity_percent}%"
...
function battery_widget:get_state()
  ...
  local sysfs = (
    file_exists(bat .. "/" .. sysfs_names.charging.rate) and sysfs_names.charging
    or sysfs_names.discharging
  )

  -- If there is no battery on this machine.
  if not sysfs.state then
    return nil
  end
...
function battery_widget:update()
  ...
  -- AC/battery prefix
  ctx.AC_BAT = (
    ctx.ac_state == 1 and lookup_by_limits(self.ac_prefix, ctx.percent)
    or lookup_by_limits(self.battery_prefix, ctx.percent)
    or "Err!"
  )

  -- Colors
  ctx.color_on, ctx.color_off = color_tags(lookup_by_limits(self.percent_colors, ctx.percent))
  ...
  if naughty then
    if ctx.state == "discharging" and ctx.percent and ctx.percent <= self.alert_threshold then
      self:notify(substitute(self.alert_title, ctx), substitute(self.alert_text, ctx), self.alert_icon)
    elseif ctx.state == "full" and self.warn_full_battery then
      self:notify("Battery Full!", "Remove power cord", self.full_battery_icon)
    else
      if self.alert then
        naughty.destroy(self.alert, naughty.notificationClosedReason.dismissedByCommand)
        self.alert = nil
      end
    end
  end
